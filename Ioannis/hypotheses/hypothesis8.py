...
print(f"Keeping durations between {lower:.1f}s and {upper:.1f}s")
print("\n--- Payment Duration Stats by Method ---")
print(payments.groupby('p')['duration_seconds'].describe().round(2))
...
for method, color in zip(payment_methods, colors):
    subset = payments[payments['p'] == method]['duration_seconds']
    subset.plot.kde(ax=ax, label=method, color=color, linewidth=2)

ax.set_title('Payment Duration KDE — All Methods Overlaid', fontsize=13)
ax.set_xlabel('Duration (seconds)')
